    ...
    updateEmployee(employee) {
        ...
        return new Promise((resolve, reject) => {
            ...
            getEmpCollection((err, client, empsCollection) => {
                ...
                var newvalues = { $set: {name: employee.name, email:employee.email, 
                        phone:employee.phone, city:employee.city } };

                        const id = new ObjectId(employee._id);
                
                empsCollection.updateOne({_id:id},  newvalues, (err, doc) => {
                    if(err) {
                        reject(err);
                    }else {
                        resolve(this.getEmpById(employee._id));
                    }
                    client.close(); 
                })
    ...
    deleteEmpById(id) {
        return new Promise((resolve, reject) => {
            ...
            getEmpCollection((err, client, empsCollection) => {
                ...
                empsCollection.deleteOne({_id : id}, (err, data) => {
                    if(err) {
                        reject(err);
                    }else {
                        resolve(data);
